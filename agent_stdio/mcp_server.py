"""MCP server for the Clinical Research Assistant (stdio transport).

Three tools, all using public APIs that need no API key:
...
This file is launched by the ADK agent as a subprocess. ADK and the
server talk to each other over the process's stdin and stdout, which
is why no port or URL appears anywhere in this file.
"""
import requests
...
# A small bundled corpus so the demo works fully offline.
# In production, replace the dict with a Vector Search lookup
# over your real clinical PDFs or a call to an internal API.
GUIDELINES = {
...
if __name__ == "__main__":
    # Default transport is stdio. The ADK agent launches this
